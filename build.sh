...
SRCS=NXBootCmd/main.m
CFLAGS="-std=gnu11 -fobjc-arc -fobjc-weak -fmodules -fvisibility=hidden -Wall -O2"
FRMWKS="-framework CoreFoundation -framework Foundation -framework IOKit"
xcrun -sdk iphoneos clang $SRCS $CFLAGS -arch armv7 -o $BINDIR/nxboot.armv7
xcrun -sdk iphoneos clang $SRCS $CFLAGS -arch arm64 -o $BINDIR/nxboot.arm64
...
clang $SRCS $CFLAGS -arch x86_64 -o $BINDIR/nxboot.x86_64
mkdir -p DerivedData/bin/
...
for arch in armv7 arm64; do
    ...
    jtool --sign --inplace --ident com.mologie.NXBoot --ent NXBoot/NXBootJailbreak.entitlements $RELEASEDIR/NXBoot.app/NXBoot.$arch
done
