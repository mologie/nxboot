...
NXBOOT_CMD_LDFLAGS="-lNXBootKit -sectcreate __TEXT __intermezzo $PROJDIR/NXBoot/Payloads/intermezzo.bin"
xcrun -sdk iphoneos clang NXBootCmd/main.m $NXBOOT_CMD_CFLAGS $NXBOOT_CMD_FRAMEWORKS -L"$RELEASEDIR_IOS" $NXBOOT_CMD_LDFLAGS -arch armv7 -miphoneos-version-min=7.0 -o $BINDIR/nxboot.armv7
jtool --sign --inplace --ident com.mologie.NXBootCmd --ent NXBootCmd/NXBootCmd.entitlements $BINDIR/nxboot.armv7
xcrun -sdk iphoneos clang NXBootCmd/main.m $NXBOOT_CMD_CFLAGS $NXBOOT_CMD_FRAMEWORKS -L"$RELEASEDIR_IOS" $NXBOOT_CMD_LDFLAGS -arch arm64 -miphoneos-version-min=7.0 -o $BINDIR/nxboot.arm64
jtool --sign --inplace --ident com.mologie.NXBootCmd --ent NXBootCmd/NXBootCmd.entitlements $BINDIR/nxboot.arm64
clang NXBootCmd/main.m $NXBOOT_CMD_CFLAGS $NXBOOT_CMD_FRAMEWORKS "-L$RELEASEDIR_MACOS" $NXBOOT_CMD_LDFLAGS -arch x86_64 -mmacosx-version-min=10.10 -o $BINDIR/nxboot.x86_64
